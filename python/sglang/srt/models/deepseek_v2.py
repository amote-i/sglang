...
class DeepseekV2DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.enable_sp:
            self.layer_scatter_modes_sp_dense = LayerScatterModes.init_new(
                layer_id=layer_id,
                num_layers=1 if is_nextn else config.first_k_dense_replace,
                is_layer_sparse=self.is_layer_sparse,
                is_previous_layer_sparse=False,
                enable_sp=True,
            )
            self.layer_communicator_sp_dense = LayerCommunicator(
                layer_scatter_modes=self.layer_scatter_modes_sp_dense,
                input_layernorm=self.input_layernorm,
                post_attention_layernorm=self.post_attention_layernorm,
                allow_reduce_scatter=True,
                is_last_layer=(
                    is_nextn or (self.layer_id == self.config.first_k_dense_replace - 1)
                ),
                enable_sp=True,
            )

            self.layer_scatter_modes_sp_sparse = LayerScatterModes.init_new(
                layer_id=layer_id,
                num_layers=1 if is_nextn else config.num_hidden_layers,
                is_layer_sparse=self.is_layer_sparse,
                is_previous_layer_sparse=is_previous_layer_sparse,
            ...
            self.layer_communicator_sp_sparse = LayerCommunicator(
                layer_scatter_modes=self.layer_scatter_modes_sp_sparse,
                input_layernorm=self.input_layernorm,
                post_attention_layernorm=self.post_attention_layernorm,
                allow_reduce_scatter=True,
                is_last_layer=(
    ...
    def get_layer_communicator(self, is_extend: bool = False):
        ...
        if self.is_layer_sparse:
            return self.layer_communicator_sp_sparse
        else:
            return self.layer_communicator_sp_dense
